...
public class JeuPuzzle extends Jeu<Puzzle> {
	...
	@Override
	public void joue(Joueur<Puzzle> j,int[] arguments) throws NonDoableError {
		if(!p.correct(arguments[0], arguments[1])) {
			throw new NonDoableError("Les premieres coordonnees ne sont pas correctes");
		}
		else if(!p.correct(arguments[2], arguments[3])) {
			throw new NonDoableError("Les secondes coordonnees ne sont pas correctes");
		}
		else if(arguments[0]==arguments[2] && arguments[1]==arguments[3]) {
			throw new NonDoableError("Les coordonnees doivent etre differentes");
		}
		...
		if (!this.p.setE(p1, arguments[2], arguments[3])) {
			throw new NonDoableError("La premiere piece n'est pas posable ici");
		}
		if (this.p.setE(p2, arguments[0], arguments[1])) {
			return;
		}
		this.p.TakeE(arguments[2], arguments[3]);
		throw new NonDoableError("La seconde piece n'est pas posable ici");
	}
